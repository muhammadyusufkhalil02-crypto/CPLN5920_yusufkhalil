...
home_prices <- st_read("data/opa_properties_public.geojson")
...
playgrounds <- st_read("data/PPR_Playgrounds.shp")
...
call_311_CT <- call_311 %>% 
  filter(subject == "Illegal Dumping") %>% 
  st_transform(2272)

call_311_CT <- st_join(acs_data2, call_311_CT) %>% 
  group_by(NAME) %>% 
  summarise(
    n = n()
  ) %>% 
...
crime_selected<- crime %>% 
  filter(crime_type == c("Aggravated Assault Firearm", "Burglary Residential", "Homicide - Criminal", "Motor Vehicle Theft", "Rape")) %>% 
  st_transform(2272)

crime_join <- st_join(acs_data2, crime_selected) %>% 
  group_by(NAME, crime_type) %>% 
  summarise(
    n = n()
  )

##figured we may need census tract spatial data eventually 

census_tracts <- tracts(state = "PA", cb = TRUE)
census_tracts_phl<- census_tracts %>% 
  filter(NAMELSADCO== "Philadelphia County")

##creating a buffer for transit stops and adding it to home_price df 

transit<- transit %>% st_transform(2272) 

transit_buffered <- st_buffer(transit, 1320)
home_prices$transit.Buffer <- lengths(st_within(transit_buffered, home_prices))
...
# Remove empty/NA geometries first
home_prices <- home_prices %>% filter(!st_is_empty(geometry))
transit <- transit %>% filter(!st_is_empty(geometry))

home_coords <- st_coordinates(home_prices)
transit_coords <- st_coordinates(transit)
...
nn <- get.knnx(transit_coords, home_coords, k = 50)
home_prices$transit_count_1320 <- rowSums(nn$nn.dist <= 1320)

nnn <- get.knnx(transit_coords, home_coords, k = 5)

home_prices$transit_avg_dist <- rowMeans(nnn$nn.dist)
...
hist(home_prices$market_value)

building_type <- home_prices %>% 
...
## Realistic sale prices based on quantiles :)
quantile(home_price_resi$sale_price)
...
chart <- ggplot(home_price_resi_filter, aes(x = sale_price)) +
...
plot(home_price_resi_filter$sale_price, home_price_resi_filter$number_of_bedrooms)
plot(home_price_resi_filter$sale_price, home_price_resi_filter$number_of_rooms)
plot(home_price_resi_filter$sale_price, home_price_resi_filter$number_of_bathrooms)
```
...
##avg sale price per census tract
sale_price_census <- acs_data2 %>%
  st_join(home_price_resi_filter, by=st_within) %>% 
  group_by(NAME) %>% 
...
ggplot() +
  geom_sf(data = acs_data2, fill = "white", color = "gray") +
  geom_sf(data = sale_price_census, aes(fill = avg_price), colour = "white") +
    scale_fill_viridis_c(
      name = "% Vulnerable Tract",
      option = "mako", direction=-1 ) +

  labs(
    title = "Averge Sale Price Per Cenus Tract",
...
ggplot() +
  ...
  labs(
    title = "Average Sale Price Per Census Tract",
    caption = "Source: ACS 2023"
  ) +
  theme_void()
...
tm_shape(sale_price_census) +
  tm_polygons(fill = "avg_price",
              fill.scale = tm_scale_intervals(style="jenks"),
              fill.legend = tm_legend(title = "Philadelphia Average Sale Price in $"),
...
tm_shape(sale_price_census) +
  tm_polygons(fill = "avg_price",
              fill.scale = tm_scale_intervals(style = "jenks"),
              fill.legend = tm_legend(title = "Avg Sale Price ($)",
                                      position = tm_pos_out("right", "center"))) +
  tm_title("Average Sale Price by Census Tract") +
  tm_credits("Source: Philadelphia Property Sales, 2023-2025",
...
plot(home_price_resi_filter$sale_price, home_price_resi_filter$transit_avg_dist) 

ggplot(home_price_resi_filter, aes(x = transit_avg_dist, y = sale_price)) +
...
nn_crime <- get.knnx(crime_coords, home_coords, k = 5)
home_prices <- home_prices[home_clean,]
home_prices$crime_avg_dist <- rowMeans(nn_crime$nn.dist)

crime_to_join <- home_prices %>%
  select(crime_avg_dist, geometry)
home_price_resi_filter <- crime_to_join %>%
  st_join(home_price_resi_filter, by = st_intersect) 
