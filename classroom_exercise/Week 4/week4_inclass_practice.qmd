...
my_county <- pa_counties %>%
  filter(COUNTY_NAM == "PHILADELPHIA")  # Change "CENTRE" to your county
...
**Question:** Why is there a difference of 1? What does this tell you about the difference between `st_touches` and `st_intersects`? It is because st_intersects includes the centroid we are predicating. 
...
my_county_hospitals <- hospitals_with_county %>%
  filter(COUNTY_NAM == "PHILADELPHIA")  # Change to match your county
...
hospital_service_areas <- my_county_hospitals_proj %>%
  st_buffer(dist = 7920)  # 15 miles in feet for PA State Plane
...
ggplot() +
  ...
  labs(
    ...
    subtitle = "Red points = Hospitals, Blue areas = 1.5-mile service zones"
  ) +
  theme_void()
```
...
55.6% of Residents in Philadelphia County live 1.5km away from a hospital 
